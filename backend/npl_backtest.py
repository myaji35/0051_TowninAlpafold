...
실행:
  ...
  python -m backend.npl_backtest --real-csv path/to/auction_results.csv  # 실데이터
"""
...
# ── 실데이터 보정계수 저장 경로 (scorer가 로드하는 단일 진실 소스) ──────────────
...
# ── 실데이터 CSV 필수 컬럼 ──────────────────────────────────────────────────
_REQUIRED_COLUMNS = {
    "asset_id", "predicted_p50", "actual_auction_price",
    "actual_recovery", "region_code", "collateral_type",
}


def load_auction_results(csv_path: str) -> list[dict]:
    """
    실 경매결과 CSV를 파싱하여 백테스트 입력 리스트 반환.

    ⚠ 실데이터 전용 — 합성 데이터로 이 함수를 호출하지 말 것.
      합성 검증은 make_ground_truth() + run()을 사용한다.
    ...
    선택 컬럼:
      actual_recovery_months

    결측/형식오류 행은 skip하고, skip_count를 집계에 포함.

    Args:
        csv_path: 실 경매결과 CSV 파일 경로

    Returns:
        파싱 성공 행 리스트 (각 행은 backtest() 호환 dict)
    ...
    if not os.path.exists(csv_path):
        raise FileNotFoundError(f"실 경매결과 CSV 없음: {csv_path}")
    ...
    with open(csv_path, newline="", encoding="utf-8-sig") as f:
        ...
        for i, row in enumerate(reader, start=2):  # 행 번호(헤더=1)
            try:
                predicted_p50 = float(row["predicted_p50"])
                actual_recovery = float(row["actual_recovery"])
                actual_auction_price = float(row["actual_auction_price"])
                if predicted_p50 <= 0 or actual_recovery < 0:
                    skip_count += 1
                    continue
                results.append({
                    "asset_id": row["asset_id"].strip(),
                    "predicted_p50": predicted_p50,
                    "actual_auction_price": actual_auction_price,
                    "actual_recovery": actual_recovery,
                    "region_code": row["region_code"].strip(),
                    "collateral_type": row["collateral_type"].strip(),
                    "actual_recovery_months": (
                        int(row["actual_recovery_months"])
                        if row.get("actual_recovery_months", "").strip()
                        else None
                    ),
                    "region_tier": region_tier(row["region_code"].strip()),
                    "gt_source": "real",  # ⚠ 실 경매결과 — 합성 아님
                    "skip": False,
                })
            except (ValueError, KeyError):
                skip_count += 1

    if not results and skip_count == 0:
        raise ValueError("유효 데이터 행 없음 (헤더만 있는 파일)")

    print(
        f"[load_auction_results] 파싱 완료: {len(results)}건 유효, "
        f"{skip_count}건 skip (결측/형식오류)"
    )
    return results
...
def save_correction(
    correction: dict,
    path: str = _CORRECTION_PATH,
) -> None:
    """
    실 백테스트로 산출된 보정계수를 JSON으로 저장.

    ⚠ 실데이터 전용 — 합성 run()에서는 이 함수를 호출하지 않는다.
      source 필드가 "real_backtest"인 경우에만 scorer가 로드한다.
    ...
    저장 구조:
      {
        ...
        "by_collateral_type": {ct: float, ...},
        "by_region_tier": {tier: float, ...}
      }

    Args:
        correction: derive_correction() 반환값
        path: 저장 경로 (기본: backend/data/auction_correction.json)
    """
    if correction.get("error"):
        raise ValueError(f"보정계수 산출 실패로 저장 불가: {correction['error']}")

    payload = {
        "source": "real_backtest",  # ⚠ 실데이터 전용 — 합성 보정 저장 금지
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "n_samples": correction.get("n", 0),
        "global_correction": correction.get("global_correction"),
        "bias_direction": correction.get("bias_direction"),
        "by_collateral_type": correction.get("by_collateral_type", {}),
        "by_region_tier": correction.get("by_region_tier", {}),
    }
    ...
    print(f"[save_correction] 보정계수 저장 완료: {path}  (n={payload['n_samples']})")


def run_real_backtest(csv_path: str) -> dict:
    """
    실 경매결과 CSV → 백테스트 → 보정계수 산출 → JSON 저장 → 결과 dict 반환.

    ⚠ 실데이터 전용 파이프라인 — 합성 데이터 투입 금지.
      실 경매결과 CSV가 없으면 T2 조달 완료 후 실행한다.

    파이프라인:
      load_auction_results → backtest → coverage_calibration
      → derive_correction → save_correction

    Args:
        csv_path: 실 경매결과 CSV 경로
    ...
    Returns:
      {
        ...
        "calibration": {...},
        "correction": {...},
        "correction_saved_to": path
      }
    """
    actuals = load_auction_results(csv_path)
    n = len(actuals)
    ...
    save_correction(cor, path=_CORRECTION_PATH)
    ...
    report = {
        ...
        "calibration": cal,
        "correction": cor,
        "correction_saved_to": _CORRECTION_PATH,
    }
    return report
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--real-csv", metavar="PATH",
        help="⚠ 실데이터 전용: 실 경매결과 CSV 경로 → 보정계수 자동 저장",
    )
    args = parser.parse_args()

    if args.real_csv:
        # 실데이터 파이프라인 (T2 조달 완료 후)
        result = run_real_backtest(args.real_csv)
        _print_report({
            "meta": {**result["meta"], "warning": "실 경매결과 기반 백테스트"},
            "error_metrics": result["error_metrics"],
            "calibration": result["calibration"],
            "correction": result["correction"],
        })
        print(f"\n보정계수 저장: {result['correction_saved_to']}")
    else:
        # 합성 검증 (기본)
