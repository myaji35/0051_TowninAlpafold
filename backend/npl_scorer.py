...
AUCTION_RATE = {"p10": 0.68, "p50": 0.82, "p90": 0.95}
...
def evaluate_buy(inp: dict) -> dict | None:
    ...
    months = _num(inp.get("recovery_months")) or DEFAULT_RECOVERY_MONTHS

    deduction = _num(inp.get("senior")) + _num(inp.get("tax")) + _num(inp.get("deposit"))
    net = {k: max(0.0, appraisal * AUCTION_RATE[k] - deduction) for k in ("p10", "p50", "p90")}
    ...
    senior_total = _num(inp.get("senior")) + _num(inp.get("tax")) + _num(inp.get("deposit"))
    return {
        "eval_type": "buy",
        ...
        "confidence": 0.60,
        "seniority_warning": senior_total >= claim * 0.9,
    }
