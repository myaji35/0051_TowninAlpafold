"""smoke test: LocalData 사업체 ETL wedge (의정부 금오동 × 현재월).

stdlib only — 외부 의존성 없음.
실행: python scripts/test_etl_localdata_smoke.py
"""
...
class TestLocaldataBizETL(unittest.TestCase):

    def test_module_loads(self):
        """etl/localdata_biz 모듈이 임포트 오류 없이 로드되어야 한다."""
        mod = importlib.import_module("etl.localdata_biz")
    ...
    def test_dry_run_success(self):
        ...
        from etl.localdata_biz import run
    ...
    def test_no_api_key_blocked(self):
        """LOCALDATA_API_KEY 미설정 + dry_run=False → status=blocked 이어야 한다."""
        env_backup = os.environ.pop("LOCALDATA_API_KEY", None)
        try:
            from etl import localdata_biz
            result = localdata_biz.run(dry_run=False)
            self.assertEqual(result.get("status"), "blocked",
                             f"API 키 없는데 blocked 아님: {result}")
        finally:
            if env_backup is not None:
                os.environ["LOCALDATA_API_KEY"] = env_backup

    def test_output_format(self):
        """저장된 JSON에 필수 필드 5개가 모두 존재해야 한다."""
        from etl.localdata_biz import run, WEDGE_ADM_CD, WEDGE_PERIOD, OUTPUT_DIR
        run(dry_run=True)
        ...
        self.assertEqual(data["dataset_key"], "localdata_biz")
